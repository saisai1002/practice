...
class Content extends Model{
    ...
    public function getUrlAttr($value,$data){
        ...
        if ($url_model == 2) {
            if($data['filename'] && is_string($data['filename'])){
                return $res[$data['id']] = url('index/content/detail',['filename'=>$data['filename'],'category_id'=>$data['category_id']]);
            }else{
                return $res[$data['id']] = url('index/content/detail',['id'=>$data['id'],'category_id'=>$data['category_id']]);
            }
        }else if($url_model == 1){
            if($data['filename'] && is_string($data['filename'])){
    ...
    public function getPrevAttr($value,$data){
        ...
        return $res[$data['id']] = \app\content\model\Content::where($where) -> field(true) -> find()?:[];
    }
    // 获取下一篇
    public function getNextAttr($value,$data){
        ...
        return $res[$data['id']] = \app\content\model\Content::where($where) -> field(true) -> order('id desc') -> find()?:[];
    }

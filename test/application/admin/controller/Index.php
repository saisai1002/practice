...
class Index extends \app\admin\controller\Common{
    ...
    public function password(){
        if (request()->isGet()) {
            ...
            if (is_file($file) && is_writable($file)) {
                ...
                $old = include $file;
                $salt = $old['email'] . ' love ebcms forever!';
                ...
                if ($pwd == $old['password']) {
                    ...
                    file_put_contents($file, '<?php return '.var_export($admin,true).';');
                    $this -> success('修改成功！');
                }else{
                    $this -> error('密码错误');
                }
            }else{
                $this -> error('权限错误：'.$file);
            }
    ...
    public function ueditor(){
        ...
        $config = array_merge($config,(array)\ebcms\Config::get('ueditor_upload'));

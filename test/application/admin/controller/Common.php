...
class Common extends Controller
{
    ...
    protected function getform($k = array(), $l = array())
    {
        ...
        if ($n = \think\Db::name('form')->where($q)->find()) {
            ...
            $r = \app\admin\model\Formfield::where($q)->field(true)->order('sort desc,id asc')->select();
            $s = [];
            foreach ($r as $t => $u) {
                $v = [];
                $v['id'] = $u['id'];
            ...
            if (isset($l['ext_id']) && $l['ext_id']) {
                ...
                if ($x = \app\admin\model\Extendfield::where($q)->field(true)->order('sort desc,id asc')->select()) {
                    $y = [];
                    foreach ($x as $t => $z) {
                        $v = [];
                        $v['id'] = $z['id'];
